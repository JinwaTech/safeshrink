...
def _read_ooxml_text(path):
    ...
    # .xls (OLE2) 格式必须用 xlrd，zipfile 会误打开
    ...
    try:
        ...
        # zipfile 失败 → 可能是 .xls (OLE2) 格式，尝试 xlrd
...
class SlimTab(QWidget):
    ...
    def create_text_options(self):
        ...
        self.slim_title = QLabel("压缩选项")

        self.file_content_title = QLabel("文件内容")
        self.file_content_title.setStyleSheet("font-weight: 600; font-size: 14px;")

        layout.addWidget(self.slim_title)
    ...
    def create_image_options(self):
        ...
        self.img_title = QLabel("图片压缩选项")

        self.img_title.setStyleSheet("font-weight: 600; font-size: 14px;")

        layout.addWidget(self.img_title)
        ...
        self.quality_label = QLabel("图片压缩强度:")
        ...
        self.size_group = QGroupBox("尺寸限制（可选）")

        self.size_group.setMinimumHeight(120)
        ...
        size_input_layout.addWidget(self.width_label)
        ...
        size_input_layout.addWidget(self.height_label)
    ...
    def create_content_section(self):
        ...
        layout.addWidget(self.file_content_title)
    ...
    def on_format_changed(self, index):
        ...
        if self.current_mode == 'image' and mode == 'ssd':
    ...
    def load_file_content(self, path):
        ...
        try:
            ...
            QMessageBox.warning(self, _("错误"), _("无法读取文件: {}").format(e))
    ...
    def process_file(self):
        ...
        try:
            ...
            if not self.current_file:
                ...
                QMessageBox.warning(self, _("提示"), _("请先选择文件"))
            ...
            QMessageBox.critical(self, _("处理错误"), _("处理文件时发生错误:\n\n{}\n\n{}").format(str(e), traceback.format_exc()))
    ...
    def process_text_file(self):
            ...
            try:
                ...
                if not isinstance(result, str) or not result.strip():
                    ...
                    if ext == ".pdf" and not self.chk_ocr_pdf.isChecked():
                        ...
                        QMessageBox.information(self, _("需要开启 OCR"),
                ...
                self.result_label.setText(_("✅ 已转换为 SSD 格式"))



                # Show dialog

                QMessageBox.information(self, _("完成"), f"SSD 转换完成\n\nToken: ~{orig_tokens_total:,} → ~{new_tokens['total']:,}\n节省: {token_saved:,} tokens ({saved_pct}%)")
                ...
                if err_msg.startswith("NEEDS_OCR:"):
                    ...
                    QMessageBox.information(self, _("需要开启 OCR"), err_msg[10:])

                else:

                    QMessageBox.critical(self, _("错误"), _("SSD 转换失败: {}").format(e))

                return



        if mode == "deep_clean" and ext == ".docx":
            ...
            if result["success"]:
                ...
                QMessageBox.information(self, _("完成"),
                ...
                QMessageBox.critical(self, _("错误"), _("处理失败: ") + str(result.get("error", "未知错误")))
        ...
        if mode == "aggressive":
            ...
            try:
                ...
                QMessageBox.information(self, _("完成"), f"激进压缩完成\n\n原文: ~{orig_bytes/1024:.1f}KB\n压缩后: ~{new_bytes/1024:.1f}KB\n压缩率: {compress_rate}%\n格式: TXT（纯文本）")

            except Exception as e:

                QMessageBox.critical(self, _("错误"), _("处理失败: {}").format(e))

            return



        # 标准压缩：xlsx/xls/pptx 原生处理（保留格式）

        if ext in (".xlsx", ".pptx") and mode == "standard":
            ...
            if res.get("success"):
                ...
                QMessageBox.information(self, _("完成"), f"标准压缩完成\n\n原文: ~{orig_sz/1024:.1f}KB\n压缩后: ~{new_sz/1024:.1f}KB\n压缩率: {cr_pct}%\n格式: {ext.upper()}（保留原始样式）")

            else:

                QMessageBox.critical(self, _("错误"), _("处理失败: {}").format(res.get("error", "未知错误")))
        ...
        # 结构化格式跳过压缩
        STRUCTURED = {'.json', '.xml', '.html', '.htm', '.yaml', '.yml', '.csv', '.xls'}
        if ext in STRUCTURED and mode in ("标准压缩", "激进压缩"):
            QMessageBox.information(self, _("提示"),
                _("结构化数据无需压缩；如需脱敏，请使用文档脱敏"))
            return
        ...
        if mode == "standard":
            ...
            try:
                ...
                QMessageBox.information(self, _("完成"), _("压缩完成\n\n原文: ~") + str(round(orig_bytes / 1024, 1)) + _(" KB\n压缩后: ~") + str(round(new_bytes / 1024, 1)) + " KB\n压缩率: " + str(compress_rate) + "%")

            except Exception as e:

                QMessageBox.critical(self, _("错误"), f"处理失败: {e}")
    ...
    def compress_image(self):
        ...
        try:
            ...
            if result['success']:
                ...
                QMessageBox.critical(self, _("错误"), _("压缩失败: {}").format(result.get("error")))



        except Exception as e:

            QMessageBox.critical(self, _("错误"), _("压缩失败: {}").format(e))
    ...
    def process_image_ocr(self):
        ...
        try:
            ...
            if md_content.strip():
                ...
                self.result_label.setText(_("✅ OCR 识别完成！"))
                ...
                self.result_label.setText(_("⚠️ 未识别出文字"))
            ...
            self.result_label.setText(_("⚠️ OCR 处理出错"))

            QMessageBox.critical(self, _("错误"), _("OCR 处理失败: {}").format(e))
        ...
        if hasattr(self, 'deep_cleaned_path') and self.deep_cleaned_path and os.path.exists(self.deep_cleaned_path):
            ...
            if file_path:
                ...
                try:
                    ...
                    QMessageBox.critical(self, _("错误"), _("保存失败: {}").format(e))
    ...
    def save_text(self):
        ...
        if self.current_mode == 'image':
            ...
            elif mode == "deep_clean" and Path(self.current_file).suffix.lower() == ".docx":
        ...
        if file_path:
            ...
            try:
                ...
                if self.current_mode == 'image' or hasattr(self, '_orig_tokens') or mode == "ssd":
                ...
                QMessageBox.critical(self, _("错误"), f"保存失败: {e}")
    ...
    def update_language(self, lang):
        ...
        if hasattr(self, 'slim_title'):
            self.slim_title.setText(_('压缩选项'))
        if hasattr(self, 'img_title'):
            self.img_title.setText(_('图片压缩选项'))
        if hasattr(self, 'file_content_title'):
            self.file_content_title.setText(_('文件内容'))
        if hasattr(self, 'effect_label'):
            self.effect_label.setText(_('效果：基础清理 + 去除重复字符 + 移除多余空格'))
        if hasattr(self, 'quality_label'):
            self.quality_label.setText(_('图片压缩强度:'))
        if hasattr(self, 'width_label'):
            self.width_label.setText(_('最大宽度:'))
        if hasattr(self, 'height_label'):
            self.height_label.setText(_('最大高度:'))
        ...
        if hasattr(self, 'format_combo'):
            ...
            self.format_combo.addItems([_('标准压缩'), _('激进压缩'), _('深度清理'), _('转换为SSD')])
            self.format_combo.setCurrentIndex(idx)
        if hasattr(self, 'img_format_combo'):
            idx = self.img_format_combo.currentIndex()
            self.img_format_combo.clear()
            self.img_format_combo.addItems([_('图片压缩'), _('识别为SSD')])
            self.img_format_combo.setCurrentIndex(idx)
        ...
        if hasattr(self, 'chk_resize_image'):
            ...
            self.chk_resize_image.setToolTip(_('当图片超过设定尺寸时自动缩小'))
        if hasattr(self, 'chk_embed_images'):
            self.chk_embed_images.setText(_('嵌入图片（Base64）'))
        if hasattr(self, 'chk_ocr_images'):
            self.chk_ocr_images.setText(_('OCR 文字识别（提取图片中的文字）'))
        if hasattr(self, 'chk_ocr_pdf'):
            self.chk_ocr_pdf.setText(_('对PDF文件进行OCR'))
        ...
        if hasattr(self, 'img_chk_ocr'):
            self.img_chk_ocr.setText(_('OCR 文字识别（提取图片中的文字）'))

        # GroupBox
        if hasattr(self, 'size_group'):
            self.size_group.setTitle(_('尺寸限制（可选）'))

        # ToolTip & Placeholder
        if hasattr(self, 'format_combo'):
            self.format_combo.setToolTip(_('标准压缩：基础文本压缩\n深度清理：移除注释、空行等\n深度清理：只清理格式问题'))
        if hasattr(self, 'img_format_combo'):
            self.img_format_combo.setToolTip(_('图片压缩：压缩图片文件大小\n识别为SSD：自动OCR识别，输出.ssd文件'))
        if hasattr(self, 'text_edit'):
            self.text_edit.setPlaceholderText(_('文件内容将在此处显示...'))
