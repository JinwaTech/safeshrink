...
| 源码保护 | Cython 编译核心模块为 .pyd（11 个模块） |
...
├── slim_tab.py            # 文档减肥 Tab（Cython .pyd）
├── sanitize_tab.py        # 脱敏 Tab
├── batch_tab.py           # 批量处理 Tab（Cython .pyd）
├── safe_shrink.py         # 核心逻辑（Cython .pyd）
├── safe_shrink_gui.py     # GUI 核心（Cython .pyd）
├── batch_processor.py     # 批量处理器（Cython .pyd）
├── format_to_ssd.py       # SSD 格式转换（Cython .pyd）
├── sanitize_ssd.py        # SSD 脱敏（Cython .pyd）
├── ssd_embed_images.py    # SSD 图片嵌入（Cython .pyd）
├── file_status.py         # 文件状态管理（Cython .pyd）
├── _ooxml_to_ssd.py       # Office→SSD 转换（Cython .pyd）
├── result_compare_dialog.py # 结果对比对话框
...
├── requirements.lock.txt  # 精确依赖锁定（51 行）
├── LICENSES_THIRD_PARTY.txt # 第三方许可证
├── assets/                # 图标等资源
└── docs/                  # 内部文档（不公开）
    └── README.internal.md # 内部用户指南
```
...
1. **文件名后缀**：输入文件添加 `_减肥`、`_脱敏`、`_SSD` 等后缀
...
### 3.2 format_to_ssd.py（Cython 编译）

Office/PDF → SSD（Markdown）格式转换。支持 OCR（Tesseract）预处理。

**关键逻辑**：
- PDF 扫描件检测：若 `ssd_text` 为空字符串且 `ocr_pdf=False`，抛出 `NEEDS_OCR` 特殊错误
- markitdown 不可用时，PDF 应提示"MarkItDown 不可用"而非直接报 NEEDS_OCR
...
| 命令 | 功能 | 关键参数 |
|------|------|---------|
| `slim` | 单文件减肥 | `-m/--mode` (standard/aggressive/deep-clean/ssd) |
| `batch-slim` | 批量减肥 | 自动映射 mode→options |
| `convert` | 格式转换 | `--format` (ssd/txt/md), `--ocr` |
| `compress-image` | 图片压缩 | `--quality`, `--max-width`, `--max-height` |
| `batch-convert` | 批量格式转换 | 同 `convert` + 批量参数 |
| `batch-compress-image` | 批量图片压缩 | 同 `compress-image` + 批量参数 |
...
| 26 | 单文档脱敏 [Errno 22] Invalid argument | `load_file_content()` 未设置 `self._current_file_path`，导致 `_sanitize_native_xlsx()` 重载 Excel 时路径无效 | 添加 `self._current_file_path = path` |
| 27 | Cython 循环依赖 | `safe_shrink.py` 自引用导入 `from safe_shrink import read_file` | 删除自引用导入（`read_file` 在同模块第 793 行已定义） |
...
#### 源码保护
- **Cython 编译**：11 个核心模块编译为 .pyd（safe_shrink、safe_shrink_gui、batch_processor、batch_tab、format_to_ssd、sanitize_ssd、slim_tab、sanitize_tab、ssd_embed_images、file_status、_ooxml_to_ssd）
- **GitHub 清理**：历史版本（v1.0.0~v1.2.0）已删除，仅保留 v1.2.1 Release；删除 44 个测试/临时/备份文件；`docs/` 目录清理
- **分支策略**：创建 `release/v1.2.1` 分支，删除 11 个 .py 源文件，发布 v1.2.1 Release

#### 核心修复
- **单文档脱敏 `[Errno 22] Invalid argument`**：`load_file_content()` 未设置 `self._current_file_path`，导致 `_sanitize_native_xlsx()` 重载 Excel 时路径无效。修复：添加 `self._current_file_path = path`
- **Cython 循环依赖**：`safe_shrink.py` 第 3026 行 `from safe_shrink import read_file` 导致编译失败。修复：删除自引用导入（`read_file` 在同模块第 793 行已定义）

#### 新增功能
- **CLI 命令体系**：`slim`（含 `-m/--mode`）、`batch-slim`、`convert`（ssd/txt/md）、`compress-image`（quality/max-width/max-height）、`batch-convert`、`batch-compress-image`
- **`result_compare_dialog.py`**：结果对比对话框（文本对比含 Token 估算、图片对比接口）

#### 构建
- EXE 体积：20.67 MB（onedir 模式，Python 3.13 + PySide6 + Cython .pyd）
...
7. **Cython 编译前消除循环依赖**：模块内自引用导入会导致编译失败
8. **单文件脱敏需设置 `_current_file_path`**：否则重载文件时路径无效
9. **markitdown API 变更需追踪**：`MarkItDown().convert()` 而非 `MarkItDown.convert()`
10. **GBK→UTF-8 编码膨胀**：文本文件"越减越大"的根本原因，`saved_bytes` 判断需区分 direct_write 模式
...
| 文件 | 说明 |
|------|------|
| `safe_shrink.py` | 核心减肥/脱敏逻辑 |
| `safe_shrink_gui.py` | GUI 核心逻辑 |
| `batch_processor.py` | 批量处理器 |
| `batch_tab.py` | 批量处理 Tab 逻辑 |
| `format_to_ssd.py` | SSD 格式转换 |
| `sanitize_ssd.py` | SSD 脱敏 |
| `slim_tab.py` | 单文件减肥 Tab 逻辑 |
| `sanitize_tab.py` | 脱敏 Tab 逻辑 |
| `ssd_embed_images.py` | SSD 图片嵌入 |
| `file_status.py` | 文件状态管理 |
| `_ooxml_to_ssd.py` | Office→SSD 转换 |

**未编译文件**（UI 框架/配置类）：
`main_window_v2.py`, `settings_tab.py`, `history_tab.py`, `history_manager.py`, `theme_manager.py`, `result_compare_dialog.py`, `translations.py`, `assets_rc.py`

**保护策略**：
- GitHub 历史版本（v1.0.0~v1.2.0）已删除
- `release/v1.2.1` 分支仅保留 16 个公开文件
- 核心算法模块编译为 .pyd 二进制，源码 .py 已删除
