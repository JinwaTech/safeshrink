...
class SlimTab(QWidget):
    file_processed = pyqtSignal(str, str)  # 原路径, 处理后内容
    ...
    def load_settings(self):
        ...
        try:
            ...
            if hasattr(self, 'img_quality_slider'):
                self.img_quality_slider.setValue(int(settings.get('image_quality', 85)))
        except Exception:
            pass
    ...
    def create_text_options(self):
        ...
        self.img_quality_value = QLabel("85%")
    ...
    def process_text_file(self):
        ...
        if mode == "转换为SSD":
            try:
                ...
                result = convert_format_to_ssd(self.current_file)
        ...
        if mode == "深度清理" and ext == '.docx':
            ...
            try:
                ...
                # 显示对比对话框
                self.show_compare_dialog_text(orig_len, new_len)
        ...
        try:
            ...
            if result['success']:
                ...
                if saved > 0:
                    ...
                    self.result_label.setText(f"⚠️ 图片已是最优状态，无法进一步压缩")

                self.btn_save.setEnabled(True)
                self.btn_undo.setEnabled(True)  # 启用撤销按钮
                self.compressed_path = temp_output

                # 显示对比对话框
                self.show_compare_dialog_image(result)
            else:
                QMessageBox.critical(self, "错误", f"压缩失败: {result['error']}")
