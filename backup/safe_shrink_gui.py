...
def compress_image_gui(input_path, output_path=None, quality=85, max_width=None, max_height=None):
    """
    GUI 图片压缩接口
...
def convert_format_to_ssd(file_path):
    '''将文档转换为 SSD 格式（使用 MarkItDown v2）'''
    try:
        result = convert_to_ssd_v2(file_path)
        return result
    except Exception as e:
        return f'转换失败: {e}'
