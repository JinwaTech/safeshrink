...
class SanitizeTab(QWidget):
    ...
    def create_types_section(self):
        ...
        self.sanitize_type_title = QLabel("脱敏类型")

        self.sanitize_type_title.setStyleSheet("font-weight: 600; font-size: 14px;")

        layout.addWidget(self.sanitize_type_title)
    ...
    def update_language(self, lang):
        ...
        if hasattr(self, 'sanitize_type_title'):
            self.sanitize_type_title.setText(_('脱敏类型'))
        ...
        if hasattr(self, 'radio_mask'):
            ...
            self.radio_mask.setToolTip(_('将敏感信息替换为 ***'))
        if hasattr(self, 'radio_tags'):
            self.radio_tags.setText(_('语义标签 (<PERSON_NAME.1>)'))
            self.radio_tags.setToolTip(_('用语义标签替代,大模型能理解类型但看不到内容'))

        # 个人敏感信息
        if hasattr(self, 'chk_phone'):
            self.chk_phone.setText(_('手机号 (138****8888)'))
        if hasattr(self, 'chk_email'):
            self.chk_email.setText(_('邮箱 (ab***@domain.com)'))
        if hasattr(self, 'chk_idcard'):
            self.chk_idcard.setText(_('身份证号 (3301***********4)'))
        if hasattr(self, 'chk_bank'):
            self.chk_bank.setText(_('银行卡号 (1234****5678)'))
        if hasattr(self, 'chk_ip'):
            self.chk_ip.setText(_('IP地址 (xxx.xxx.xxx.xxx)'))
        if hasattr(self, 'chk_passport'):
            self.chk_passport.setText(_('护照号 (G********1)'))
        if hasattr(self, 'chk_mac'):
            self.chk_mac.setText(_('Mac 地址'))
        if hasattr(self, 'chk_imei'):
            self.chk_imei.setText(_('IMEI 设备号'))
        if hasattr(self, 'chk_plate'):
            self.chk_plate.setText(_('车牌号 (浙A*****5)'))
        if hasattr(self, 'chk_social'):
            self.chk_social.setText(_('社保卡号'))

        # 商业敏感信息
        if hasattr(self, 'chk_credit'):
            self.chk_credit.setText(_('社会信用代码 (9133***XXXX)'))
        if hasattr(self, 'chk_license'):
            self.chk_license.setText(_('营业执照号'))
        if hasattr(self, 'chk_contract'):
            self.chk_contract.setText(_('合同编号 (HT****)'))
        if hasattr(self, 'chk_amount'):
            self.chk_amount.setText(_('投标/成交价 (¥***)'))
        if hasattr(self, 'chk_phone2'):
            self.chk_phone2.setText(_('固定电话'))
        if hasattr(self, 'chk_account_permit'):
            self.chk_account_permit.setText(_('开户许可证号'))
        if hasattr(self, 'chk_purchase_order'):
            self.chk_purchase_order.setText(_('采购/订单编号'))
        if hasattr(self, 'chk_fax'):
            self.chk_fax.setText(_('传真号'))
        if hasattr(self, 'chk_employee_id'):
            self.chk_employee_id.setText(_('工号/学号'))
        if hasattr(self, 'chk_project_code'):
            self.chk_project_code.setText(_('项目代号'))
        if hasattr(self, 'chk_postal'):
            self.chk_postal.setText(_('邮编'))

        # 专用类型
        if hasattr(self, 'chk_docnum'):
            self.chk_docnum.setText(_('公文份号 (No******)'))
        if hasattr(self, 'chk_doclevel'):
            self.chk_doclevel.setText(_('密级标注 (绝密★***年)'))
        if hasattr(self, 'chk_docref'):
            self.chk_docref.setText(_('公文文号 (〔2024〕*字第***号)'))
        if hasattr(self, 'chk_medicare'):
            self.chk_medicare.setText(_('医保卡号'))
        if hasattr(self, 'chk_medical_record'):
            self.chk_medical_record.setText(_('病历号/门诊号'))
